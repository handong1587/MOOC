...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  % gradient checking for weight-decay passed
  %ret.hid_to_class = wd_coefficient * model.hid_to_class;
  %ret.input_to_hid = wd_coefficient * model.input_to_hid;
  
  % the classification loss gradient part need to be divided by size(data.inputs, 2)!
  ret.hid_to_class = (class_prob - data.targets) * hid_output' / size(data.inputs, 2) + wd_coefficient * model.hid_to_class;
  ret.input_to_hid = model.hid_to_class' * (class_prob - data.targets) .* (hid_output .* (1-hid_output)) * data.inputs' / size(data.inputs, 2) + wd_coefficient * model.input_to_hid;

...
const int D1 = 8; // hidden layer neuron, not count bias
const int D2 = 3; // output neuron
const int D3 = 1; // output neuron
...
double X0[D0 + 1];
int Y;
double S1[D1 + 1], X1[D1 + 1];
double S2[D2 + 1], X2[D2 + 1];
double S3[D3 + 1], X3[D3 + 1];
double W1[D0 + 1][D1 + 1];
double W2[D1 + 1][D2 + 1];
double W3[D2 + 1][D3 + 1];
...
{
	char log_name[260];
	sprintf(log_name, "log_M(%d)_eta(%lf)_r(%lf).txt", D1, eta, r);
...
{
	// initialize bias
	X0[0] = X1[0] = X2[0] = X3[0] = 1; // X2[0] not used
	...
	{
		for (int j = 0; j < D3 + 1; j++)
		{
			W3[i][j] = f_rand(-r, r); // W2[][0] not used
		}
		W3[i][0] = 0;
	}
}
...
void calc_S1()
{
	for (int j = 1; j < D1 + 1; j++) // bias neuron = 1, just update weight
	{
		S1[j] = 0;
		for (int i = 0; i < D0 + 1; i++)
		{
...
void calc_S3()
{
	for (int j = 1; j < D3 + 1; j++) // bias neuron = 1, just update weight
	{
		S3[j] = 0;
		for (int i = 0; i < D2 + 1; i++)
		{
...
void fwd()
{
	...
	// hidden layer -> output
	calc_S3();
	calc_tanh_vector(S3, X3, D3 + 1);
}

void bwd()
{
	for (int i = 1; i < D3 + 1; i++) // delta2[0] for bias, output layer do not calculate this.
	{
		delta3[i] = -2 * (Y - X3[i]) * (1 - X3[i] * X3[i]);
	}
	
	for (int i = 0; i < D2 + 1; i++)
	{
		delta2[i] = 0;
		for (int j = 1; j < D3 + 1; j++)
		{
			delta2[i] += W3[i][j] * delta3[j] * (1 - X2[i] * X2[i]);
		}
	}
...
void update_weight()
{
	...
	for (int i = 0; i < D2 + 1; i++)
	{
		for (int j = 1; j < D3 + 1; j++)
		{
			W3[i][j] = W3[i][j] - eta * X2[i] * delta3[j];
		}
	}
	// update W2
	for (int i = 0; i < D1 + 1; i++)
